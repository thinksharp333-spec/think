...
Questions MUST be based on the provided text.`;
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            const response = await fetch(`https://generativelanguage.googleapis.com/v1/models/gemini-1.5-flash-latest:generateContent?key=${geminiKey}`, {
                method: 'POST',
            ...
            if (!response.ok) {
                ...
                console.warn(`[Retry] Rate limit hit. Waiting ${attempt * 10}s before retry...`);
